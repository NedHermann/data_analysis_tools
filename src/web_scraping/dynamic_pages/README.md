...
- We then use the `$executeScript()` function and input the js: `window.scrollTo(0, document.body.scrollHeight);` to scroll down to the *next bottom* of the page.
- We then sleep the session for another 1.5 seconds for the page to update with the new data.
- Then, we overwrite our previous `new_height` variable with the current height of the page.
...
### While loop cont.
Since I didn't want to scrape all 16k+ answers, I set the limit of the scrape to just pull until we hit 20 posts with this line of code:
...
If we look at the end of the `sub_heading` column, we can see that the final few characters displays the time that the answer was posted relative to the timestamp that requested it (12h). In the next section, we'll extract this into it's own column.
